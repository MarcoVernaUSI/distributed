...
#        self.n_hidden2 = 16
        ...
        with tf.variable_scope(number):
            ...
            rnn_output, self.final_state = self._lstm(self.x,'64',out_hidden_state=True)
    ...
    def _lstm(self,in_tensor, num_hidden_units, out_hidden_state=False, zero_init=False):
        ...
        if zero_init:
            ...
            outputs_state = tf.nn.dynamic_rnn(multi_cell, in_tensor, dtype=tf.float32)
    ...
    def predict(self, batch_inputs, pred_state = None):
        ...
        _,actual,new_state = self.sess.run([self.net,self.prediction,])

        if pred_state is None:
            pred_state = session.run 
