...
class Agent:
    ...
    def check_collisions_old(self, agents_list, L):   #outdated
    ...
    def check_collisions(self, agents_list, L):
        if (self.distance(0) < -self.width) and (self.distance(L)> self.width):
            return False
        for a in agents_list:
            if abs(self.distance(a))<0:
                return False
        return True
    ...
    def observe(self, agents_list, L, idx):
        ...
        # Just update with the width of the agents
        right=right - self.width * 2
        left=left - self.width * 2

      #  assert (right>0) and (left>0), "Collision"
