...
class Simulator:
    ...
    def run(self, init= None, control=None, parameter = None, goal_set = True):
        n_agents = self.N
        L = self.L
        ...
        if goal_set:
            ...
            free_space = self.L - (self.N*agent_width*2)
            # assert free_space > 0
            space_dist = free_space / (self.N + 1)
            # first agent
            ...
            if n_agents > 1:
                goal_list[n_agents-1]=self.L - (agent_width + space_dist)
        else:
            dist = L/(n_agents+1)            
            for i in range(n_agents):
